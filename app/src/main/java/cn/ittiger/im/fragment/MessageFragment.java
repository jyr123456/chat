...
public class MessageFragment extends BaseFragment implements CommonRecyclerView.OnItemClickListener, CommonRecyclerView.OnItemLongClickListener {
    ...
    public void refreshData() {
        Observable.create((Observable.OnSubscribe<List<ChatRecord>>) subscriber -> {
    ...
    public void onItemClick(HeaderAndFooterAdapter adapter, int position, View itemView) {
        if (mAdapter.getData().get(position).getFriendUsername().contains("@conference")){
           MultiUserChat multiUserChat = SmackManager.getInstance().getMultiUserChatManager().getMultiUserChat( mAdapter.getItem(position).getChatJid());
            IMUtil.startMultiChatActivity(mContext, multiUserChat);
        }else {
    ...
    @Override
    public void onItemLongClick(HeaderAndFooterAdapter adapter, int position, View itemView) {
        ChatRecord records = (ChatRecord) adapter.getItem(position);
        //弹出一个对话框，包含同意和拒绝按钮
        AlertDialog.Builder builder = new AlertDialog.Builder(getActivity());
        builder.setTitle("提示");
        builder.setMessage("确定移除会话记录" );
        //同意按钮监听事件，发送同意Presence包及添加对方为好友的申请
        builder.setPositiveButton("同意", (dialog, arg1) -> {
            DBHelper.getInstance().getSQLiteDB().delete(  DBQueryHelper.queryChatRecord(records.getUuid()));//更新数据库中的记录
            chatRecords.remove(position);
